...
class SelesaiStatusTiketViewModel : ViewModel() {
    private val tags = SelesaiStatusTiketFragment::class.java.simpleName
...
//        AndroidNetworking.get("$server/ticket/ticket-by-bidang")
    ...
    fun setData(
        ...
        var requestURL = "$server/ticket/list?status=3&bidang=$bidangId&user_id=$userId"
        if (roleId == "3" || roleId == "4") {
            requestURL = "$server/ticket/list?status=3&bidang=$bidangId&fmbm=$fmbm&user_id=$userId"
        }

        Log.d(tags, "URL_SELESAI : $requestURL")
