...
import id.co.telkomsigma.mybirawa.adapter.SelesaiStatusTiketAdapter
...
import id.co.telkomsigma.mybirawa.viewmodel.statusTiketViewModel.SelesaiStatusTiketViewModel
import kotlinx.android.synthetic.main.semua_status_tiket_fragment.*

class SelesaiStatusTiketFragment : Fragment() {

    companion object {
        fun newInstance() = SelesaiStatusTiketFragment()
    ...
    private lateinit var adapter: SelesaiStatusTiketAdapter
    private lateinit var adapterFilter: FilterStatusTiketAdapter
    private lateinit var viewModel: SelesaiStatusTiketViewModel

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        return inflater.inflate(R.layout.selesai_status_tiket_fragment, container, false)
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this, ViewModelProvider.NewInstanceFactory()).get(
            SelesaiStatusTiketViewModel::class.java
        )
        ...
        if (bundle != null) {
            ...
            Log.d(tag, "idFromBerandaSelesai : $idBidangRequest")
        ...
        viewModel.getData().observe(viewLifecycleOwner, Observer { model ->
            if (model.isEmpty()) {
                ...
                if (model[0].statusResponse.equals("true")) {
                    Log.d(tag, "statusSelesaiTiket : ${model[0].statusResponse}")
                    showLoading(false)
                    showAdapter(true)
                    adapter.setData(model)
                } else {
...
//                    Log.d(tag,"statusSelesaiTiket : ${model[0].statusResponse}")
    ...
    private fun showList() {
        adapter = SelesaiStatusTiketAdapter()
        rv_status_tiket.layoutManager = LinearLayoutManager(activity)
        rv_status_tiket.adapter = adapter

        adapter.setOnItemClickCallback(object : SelesaiStatusTiketAdapter.OnItemClickCallback {
