...
class SemuaStatusTiketViewModel : ViewModel() {

    private val tags = SemuaStatusTiketFragment::class.java.simpleName
...
//                    Log.d(tags,response.toString())
    ...
    fun setData(
        ...
        var requestURL = "$server/ticket/list?status=&bidang=$bidangIdEmty&user_id=$userId"
        if (roleId == "3" || roleId == "4") {
            requestURL =
                "$server/ticket/list?status=&bidang=$bidangIdEmty&fmbm=$fmbm&user_id=$userId"
        }
        ...
        AndroidNetworking.get(requestURL) // status : semua, baru, berjalan, selesai
            ...
            .getAsJSONObject(object : JSONObjectRequestListener {
                override fun onResponse(response: JSONObject?) {
                    Log.e(tags, "request_status_icket : $requestURL")
//                    Log.d(tags,"responseSemuaStatusTiket : ${response.toString()}")
                    ...
                    if (response?.getBoolean("status") == true) {
                        ...
                        if (arrayData.length() > 0) {
                            for (i in 0 until arrayData.length()) {
                                ...
                                // agar data yang statusnya 4(closed) tidak  muncul
                                val status = status_id.toString()
                                val stat = status.toInt()
