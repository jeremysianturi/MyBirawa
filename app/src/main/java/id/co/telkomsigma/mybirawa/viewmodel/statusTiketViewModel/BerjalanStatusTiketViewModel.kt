...
class BerjalanStatusTiketViewModel : ViewModel() {

    private val tags = BerjalanStatusTiketFragment::class.java.simpleName
...
//                    Log.d(tags,"filterBerjalan : ${response.toString()}")
    ...
    fun setData(
        ...
        var requestURL = "$server/ticket/list?status=2&bidang=$bidangId&user_id=$userId"
        if (roleId == "3" || roleId == "4") {
            requestURL = "$server/ticket/list?status=2&bidang=$bidangId&fmbm=$fmbm&user_id=$userId"
        }

        Log.d(tags, "URL_BERJALAN : $requestURL")
        ...
        AndroidNetworking.get(requestURL) // status : semua, baru, berjalan, selesai
            ...
            .getAsJSONObject(object : JSONObjectRequestListener {
                override fun onResponse(response: JSONObject?) {
//                    Log.e(tags,"responseSemuaStatusTiket : ${response.toString()}")
                    if (response?.getBoolean("status") == true) {
